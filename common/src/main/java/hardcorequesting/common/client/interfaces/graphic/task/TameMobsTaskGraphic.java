...
public class TameMobsTaskGraphic extends IconTaskGraphic<TameMobsTask.Part> {
    ...
    @Override
    protected void drawElementText(GuiGraphics guiGraphics, TameMobsTask.Part part, int index, int x, int y) {
        int tamed = task.tamed(index, playerId);
        if (tamed == part.getCount()) {
            gui.drawString(guiGraphics, Translator.translatable("hqm.tameTask.allTamed").withStyle(ChatFormatting.DARK_GREEN), x, y, 0.7F, 0x404040);
        } else {
            gui.drawString(guiGraphics, Translator.translatable("hqm.tameTask.partTames", tamed, (100 * tamed / part.getCount())), x, y, 0.7F, 0x404040);
        }
        gui.drawString(guiGraphics, Translator.translatable("hqm.tameTask.totalTames", part.getCount()), x, y + 6, 0.7F, 0x404040);
    }
